...
export default function StickyMaskReveal() {
  ...
  const maskSize = useTransform(scrollYProgress, [0, 0.6], [0, 300]);
  const clipPath = useMotionTemplate`circle(${maskSize}vw at center center)`;
  
  // Fade out foreground text as mask grows
  const foregroundOpacity = useTransform(scrollYProgress, [0, 0.2], [1, 0]);
  const foregroundScale = useTransform(scrollYProgress, [0, 0.2], [1, 1.1]);

  // Final fade out of the entire white layer to prevent "white screen gap" early
  const finalLayerOpacity = useTransform(scrollYProgress, [0.6, 0.75], [1, 0]);
  
  // Fade in background text as mask grows, and fade out at the end
  const backgroundOpacity = useTransform(scrollYProgress, [0.1, 0.3, 0.8, 0.95], [0, 1, 1, 0]);
  const backgroundScale = useTransform(scrollYProgress, [0.1, 0.45], [0.9, 1]);
